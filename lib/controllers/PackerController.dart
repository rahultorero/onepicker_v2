...
class PackerController extends GetxController {
      ...
      if (response.statusCode == 200) {
        ...
        if (detailModel.status == '200') {
          ...
          if (packerDetails.isNotEmpty) {
            showPackerDetailsBottomSheet(packerData);
          } else {
            Get.snackbar(
              'Info',
  ...
  // Show packer details bottom sheet
  void showPackerDetailsBottomSheet(PickerData packerData) {
    Get.bottomSheet(
      PackerDetailsBottomSheet(packerData: packerData),
