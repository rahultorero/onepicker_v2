...
// Reactive strings to track changes (this is what Obx observes)
  final RxString itemNameSearch = ''.obs;
  final RxString locationSearch = ''.obs;

// Filtered list
  final RxList<StockDetail> filteredStockList = <StockDetail>[].obs;
  var searchQuery = ''.obs;
  ...
  void onInit() {
    ...
    fetchPickerList();
    fetchStockList();
    ever(stockList, (_) => filterStockList());
  ...
  void filterStockList() {
    final itemQuery = itemNameSearch.value.toLowerCase().trim();
    final locQuery = locationSearch.value.toLowerCase().trim();

    if (itemQuery.isEmpty && locQuery.isEmpty) {
      filteredStockList.value = stockList;
    } else {
      filteredStockList.value = stockList.where((item) {
        final matchesName = itemQuery.isEmpty ||
            (item.itemName?.toLowerCase().contains(itemQuery) ?? false);
        final matchesLocation = locQuery.isEmpty ||
            (item.locn?.toString().toLowerCase().contains(locQuery) ?? false);
  ...
  void showStockDetailDialog(String itemName,String packing) {
    Get.dialog(
      Dialog(
        ...
        child: Container(
          width: Get.width * 0.95, // Increased from 0.8 to 0.95 for better visibility
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            Expanded(
                              flex: 4, // Increased from 3 to 4 for batch number
                              child: Text(
                                'Batch No.',
                                style: TextStyle(
                                  fontSize: 12,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['status'] == '200') {
          ...
          //   colorText: AppTheme.accentGreen,
          //   duration: const Duration(seconds: 3),
          //   snackPosition: SnackPosition.BOTTOM, // 👈 Shows snackbar from bottom
          //   margin: const EdgeInsets.all(12), // optional: adds padding from edges
          //   borderRadius: 8, // optional: makes it look cleaner
          // );
...
// API call to get pending location after submission
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['response'] != null) {
          ...
          await _handleMergerLogic(siId,trayNo);
        }
      }
    } catch (e) {
  ...
  Future<void> _handleMergerLogic(int siId,String trayNo) async {
    try {
